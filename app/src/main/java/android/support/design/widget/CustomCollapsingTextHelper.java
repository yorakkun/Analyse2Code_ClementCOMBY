...
public class FontFamilyTypefaceReadException extends Exception {
    public FontFamilyTypefaceReadException(String message) {
        super(message);
    }

    public FontFamilyTypefaceReadException(String message, Throwable cause) {
        super(message, cause);
    }
}
...
public final class CustomCollapsingTextHelper {
    ...
    private Typeface readFontFamilyTypeface(int resId) throws FontFamilyTypefaceReadException {
        ...
        try {
            ...
            throw new FontFamilyTypefaceReadException("Unable to read font family typeface: " + resId, e);
        } finally {
            a.recycle();
        }
        return null;
    ...
    private void drawTitleText(Canvas canvas) {
        ...
        boolean drawTexture = checkDrawTexture();
        ...
        applyScaleIfNecessary(canvas, x, y);
        drawMainTextOrTexture(canvas, x, y, drawTexture);
    }
    
    private boolean checkDrawTexture() {
    ...
    private void debugDraw(Canvas canvas, float y) {
        canvas.drawRect(mCurrentBounds.left, y + calculateAscent(), mCurrentBounds.right, y + calculateDescent(),
                DEBUG_DRAW_PAINT);
    }
    
    private float calculateDescent() {
    ...
    private void applyScaleIfNecessary(Canvas canvas, float x, float y) {
        if (mScale != 1f) {
            canvas.scale(mScale, mScale, x, y);
        }
    }
    
    private void drawMainTextOrTexture(Canvas canvas, float x, float y, boolean drawTexture) {
        if (drawTexture) {
