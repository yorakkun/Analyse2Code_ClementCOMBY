...
abstract class LocalPlayback(context: Context) : Playback {
    ...
    private var isManualPause: Boolean = false  // Indicator for manual pause
    ...
    private val onAudioFocusChangeListener = AudioManager.OnAudioFocusChangeListener { focusChange ->
        ...
        when (focusChange) {
            AudioManager.AUDIOFOCUS_GAIN -> {
                ...
                isManualPause = false  // Reset manual pause when focus is regained
            }
    ...
    override fun pause(fade: Boolean) {
        isManualPause = true  // Mark as manual pause
    ...
    override fun stop() {
        ...
        isManualPause = false  // Reset on stop
        giveUpAudioFocus()
        unregisterAudioNoisyReceiver()
    }
    ...
    override fun start() {
        ...
        isManualPause = false  // Reset on start
        tryToGetAudioFocus()
    ...
    private fun configurePlayerState() {
        ...
        if (currentAudioFocusState == AudioFocus.NO_FOCUS_NO_DUCK) {
            ...
            if (currentAudioFocusState == AudioFocus.NO_FOCUS_CAN_DUCK) {
                // We're permitted to play, but only if we 'duck', i.e., play softly
